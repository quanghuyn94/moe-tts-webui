...
from modules import commons, modules, attentions, utils
...
#Add self keywords where you need them
class SynthesizerTrn(nn.Module):
    ...
    def __init__(self,
        ...
        self.emotion_embedding : bool = emotion_embedding
    ...
    def setup(self):
        ...
        self.dec = Generator(self.inter_channels, self.resblock, self.resblock_kernel_sizes, self.resblock_dilation_sizes, self.upsample_rates,
                            self.upsample_initial_channel, self.upsample_kernel_sizes, gin_channels=self.gin_channels)
        self.enc_q = PosteriorEncoder(self.spec_channels, self.inter_channels, self.hidden_channels, 5, 1, 16,
                                    gin_channels=self.gin_channels)
        self.flow = ResidualCouplingBlock(self.inter_channels, self.hidden_channels, 5, 1, 4, gin_channels=self.gin_channels)

        if self.use_sdp:
            self.dp = StochasticDurationPredictor(self.hidden_channels, 192, 3, 0.5, 4, gin_channels=self.gin_channels)
        else:
    ...
    # def setup(self, emotion_embedding : bool = False):
    #     self.enc_p = TextEncoder(self.n_vocab,
    #                              self.inter_channels,
    #                              self.hidden_channels,
    #                              self.filter_channels,
    ...
    #                              self.p_dropout,
    #                              emotion_embedding)
    #     self.dec = Generator(self.inter_channels, self.resblock, self.resblock_kernel_sizes, self.resblock_dilation_sizes, upsample_rates,
    ...
    def to_speak(self, text : str, speaker_id : int, speed : float = 1, is_symbol : bool = False):
        ...
        with no_grad():
            ...
            audio : numpy.ndarray = self.infer(x_tst, x_tst_lengths, sid=sid, noise_scale=.667, noise_scale_w=0.8,
